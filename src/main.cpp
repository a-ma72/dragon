...
public:
    ...
    mutable SDL_Texture *cache_texture = nullptr;
    mutable int cache_w = 0;
    mutable int cache_h = 0;
    mutable Uint64 cache_idle = ~(Uint64)0;
    mutable float cache_alpha = -1.f;
    ...
    void draw(const SDL_FPoint& pt, float global_alpha, const SDL_Renderer* renderer) const override
    {
        ...
        else
        {
            ...
            const float blended_alpha = global_alpha * this->alpha;
            if (!cache_dirty && cache_texture &&
                cache_w == wa_width && cache_h == wa_height &&
                cache_idle == idle_ticks && cache_alpha == blended_alpha)
            {
                SDL_FRect rect = {0, 0, (float)wa_width, (float)wa_height};
                SDL_SetTextureBlendMode(cache_texture, SDL_BLENDMODE_BLEND);
                SDL_RenderTexture(const_cast<SDL_Renderer *>(renderer), cache_texture, nullptr, &rect);
                return;
            }
            ...
            cache_w = wa_width;
            cache_h = wa_height;
            cache_idle = idle_ticks;
            cache_alpha = blended_alpha;
            cache_dirty = false;
...
SDL_AppResult SDL_AppIterate(void *appstate)
{
    ...
    if (line_object && line_object->dashed && line_object->dashed_gap > 0 && line_object->width > 0 && !app->hidden && !dragging)
    {
